...
- **prun workers no longer re-run bootstrap, and there is now a safe way to reap orphaned ones** ([#26](https://github.com/yzhao062/anywhere-agents/issues/26)). `dispatch-codex.{sh,ps1}` passes `developer_instructions` telling the child to skip bootstrap and use the configuration already on disk. `dispatch-task.{sh,ps1}` was generalised from it and dropped that argument. Across 298 local prun state directories, 8% of workers spent their first command on the session-start procedure. At least one ran a full bootstrap mid-batch, which rewrites shared configuration under every later unit in the same run. Both dispatchers now pass the argument.

  `reap-orphans.{sh,ps1}` is new, and it reaps by default with `--dry-run` as the opt-in. The tool exists because a caller with no safe reaper available wrote a blanket `Get-Process -Name codex | Stop-Process -Force` and killed two live workers. A reaper that does nothing without a second flag invites that same bypass. Safety comes from the bounded target set rather than from being read-only. Only a worker whose recorded dispatcher is dead and whose identity still matches is eligible, and no process is ever located by name. This differs from `guard.py`, which requires human confirmation for arbitrary process destruction, because there the affected set cannot be inferred from the command text and here it is enumerable and test-covered.

  Worker identity is recorded with an explicit namespace tag, which took two attempts. `ps -o lstart=` does not exist on MSYS. The first implementation wrote an empty identity token on Windows and then declined to reap anything. That left the tool inert in the environment the issue came from. Git Bash `/tmp` and `%TEMP%` are also the same directory. The two dispatchers were therefore writing PIDs from different namespaces into one `worker-roots` file, kept apart only by their token formats never comparing equal. Records are now tagged `posix`, `win`, or `msys`. An `msys` record carries both identities: the MSYS pid with `/proc/<pid>/stat` field 22, and the Windows pid with its start ticks. A unit dispatched from Git Bash can therefore be reaped from PowerShell under full identity verification. A record whose scheme a reaper cannot verify is refused as `foreign-scheme`, and a pre-existing two-field record as `unknown-identity`. Both dispatchers also hash one canonical spelling of the repository path, so a single repo no longer appears as two unrelated state-directory namespaces.

  An eligible candidate reports `TERMINATED` once its recorded root and every descendant retained from a successful process-table snapshot are confirmed gone. That is a narrower claim than the tool first made, and there is deliberately no `REAPED` outcome. Four review rounds each strengthened the tree-wide proof, and each was refuted by a narrower escape. The last one needs only timing. A short-lived intermediate created after the first snapshot spawns a child and exits before the second, so neither ever appears in a scan. The gap is opened at dispatch rather than at reap. `setsid` is absent from Git for Windows, and the PowerShell dispatcher spawns through `cmd.exe` with no Job Object, so the worker never lands in a bounded container. Polling cannot prove a container empty when there is no container. `REAPED` is reserved for that containment work ([#29](https://github.com/yzhao062/anywhere-agents/issues/29)) rather than retired, so the word will mean something when it returns. Two Windows details come from the same round. Each process enumeration is now bounded by the remaining deadline. An identity query that fails is distinguished from a process that is gone, so an unqueryable survivor can no longer clear a candidate.
