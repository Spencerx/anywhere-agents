...
装一个 `anywhere-agents` CLI（`pipx install anywhere-agents` 或 `pip install`），就能不改每个项目的 YAML 来管 pack。
...
`pack add` 写到 user-level config 之后，项目还需要在 `agent-config.yaml` 里有 `rule_packs:` 条目，bootstrap composer 才能真正部署 pack。用 `pack verify` 一眼确认整个链路是否对齐：

```bash
anywhere-agents pack verify              # 只读审计
anywhere-agents pack verify --fix --yes  # 自动写入缺失的 rule_packs 条目
bash .agent-config/bootstrap.sh          # 实际部署
```
...
下一次 `bootstrap` 就会把这些 pack 应用上。bootstrap cache 在第一次跑 v0.5.0 时需要做一次 seed-refresh，详情见 [`MIGRATIONS.md`](MIGRATIONS.md)。如果 `pack add` 跑完 `bootstrap.sh` 之后看起来没生效，跑 `anywhere-agents pack verify` 一眼看清部署状态，加 `--fix` 自动写缺失的 `rule_packs:` 条目。
