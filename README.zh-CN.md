...
安装 `anywhere-agents` CLI（`pipx install anywhere-agents` 或 `pip install`）后，可以在用户级配置中管理 pack，避免在每个项目里重复编辑 YAML：
...
anywhere-agents uninstall --all       # 清理当前项目的所有 pack 输出
```

CLI 会写入 `$XDG_CONFIG_HOME/anywhere-agents/config.yaml`（POSIX）或 `%APPDATA%\anywhere-agents\config.yaml`（Windows），让你可以在不修改每个项目 YAML 的情况下管理用户级 pack 选择。v0.4.0 的 bootstrap 组合流程仍然走旧的 project-tracked / project-local / env 选择路径；把 composer 接到完整的四层解析器（user → project-tracked → project-local → `AGENT_CONFIG_PACKS`）是 v0.4.x 的后续工作。
